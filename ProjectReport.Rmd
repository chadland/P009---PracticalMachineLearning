...
The raw training data has 19622 rows of observations and 158 features (predictors). Column `X` is unusable row number. While the testing data has 20 observations and the same 158 features. There is one column of target outcome named `classe`.   

#### Data cleaning

First, extract target outcome (the activity quality) from training data in the data frame outcome. Print the various levels of outcome.   
```{r}
...
The assignment rubric asks to use data from the accelerometers on the `belt`, `forearm`, `arm`, and `dumbell`, so the features are extracted based on these keywords.   
...
This might be a conservative approach and imputation methods could have been used to filling the blanks. 
...
Check for zero variance.  Each feature should ideally be as distant as possible (as orthogonal as possible) from the other features.   
```{r}
...
A good set of features is when they do not correalte too much, especially when using regression. Some features seem to corrolate but using the tree algorithms such as decision trees or boosting trees collinarity and correlation is not a major issue. 
...
```{r}
# create answer folder if it does not exist
